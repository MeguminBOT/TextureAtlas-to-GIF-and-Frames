...
class Extractor:
    ...
    def process_directory(self, input_dir, output_dir, progress_var, tk_root, create_gif, create_webp, set_framerate, set_loopdelay, set_minperiod, set_scale, set_threshold, keep_frames, crop_option, var_delay, fnf_idle_loop):
            ...
            for filename in os.listdir(input_dir):
                if filename.endswith('.png'):
                    ...
                    if os.path.isfile(xml_path) or os.path.isfile(txt_path):
                        ...
                        future = executor.submit(self.extract_sprites, os.path.join(input_dir, filename), xml_path if os.path.isfile(xml_path) else txt_path, sprite_output_dir, create_gif, create_webp, fps, delay, period, scale, threshold, indices, frames, crop_option, var_delay, fnf_idle_loop, self.user_settings, self.current_version)
    ...
    def extract_sprites(self, atlas_path, metadata_path, output_dir, create_gif, create_webp, set_framerate, set_loopdelay, set_minperiod, set_scale, set_threshold, set_indices, keep_frames, crop_option, var_delay, fnf_idle_loop, user_settings, current_version):
        frames_generated = 0
        anims_generated = 0
        sprites_failed = 0
        try:
            ...
            animation_processor = AnimationProcessor(animations, atlas_path, output_dir, create_gif, create_webp, set_framerate, set_loopdelay, set_minperiod, set_scale, set_threshold, set_indices, keep_frames, crop_option, var_delay, fnf_idle_loop, user_settings, current_version)
