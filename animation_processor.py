...
class AnimationProcessor:
    ...
    def __init__(self, animations, atlas_path, output_dir, create_gif, create_webp, set_framerate, set_loopdelay, set_minperiod, set_scale, set_threshold, set_indices, keep_frames, crop_option, prefix, var_delay, fnf_idle_loop, user_settings, current_version):
    ...
    def save_frames(self, image_tuples, kept_frame_indices, animation_name, scale):
        ...
        for index, frame in enumerate(image_tuples):
            if index in kept_frame_indices:
                frame_filename = os.path.join(frames_folder, f"{self.prefix}_{frame[0]}.png" if self.prefix else f"{frame[0]}.png")
                frame_image = frame[1]
    ...
    def save_webp(self, images, spritesheet_name, animation_name, fps, delay, period, scale):
        ...
        webp_filename = os.path.join(self.output_dir, f"{self.prefix}_{os.path.splitext(spritesheet_name)[0]}_{animation_name}.webp" if self.prefix else f"{os.path.splitext(spritesheet_name)[0]}_{animation_name}.webp")
        scaled_images[0].save(webp_filename, save_all=True, append_images=scaled_images[1:], disposal=2, duration=durations, loop=0, lossless=True)
    ...
    def save_gif(self, images, spritesheet_name, animation_name, fps, delay, period, scale, threshold, max_size, image_tuples, current_version):
        ...
        gif_filename = os.path.join(self.output_dir, f"{self.prefix}_{os.path.splitext(spritesheet_name)[0]}_{animation_name}.gif" if self.prefix else f"{os.path.splitext(spritesheet_name)[0]}_{animation_name}.gif")
        cropped_images[0].save(gif_filename, save_all=True, append_images=cropped_images[1:], disposal=2, optimize=False, duration=durations, loop=0, comment=f'GIF generated by: TextureAtlas to GIF and Frames v{current_version}')
