        ...
        for filename in os.listdir(input_dir):
            if filename.endswith('.png'):
                ...
                if os.path.isfile(xml_path) or os.path.isfile(txt_path):
                    ...
                    future = executor.submit(extract_sprites, os.path.join(input_dir, filename), xml_path if os.path.isfile(xml_path) else txt_path, sprite_output_dir, create_gif, create_webp, fps, delay, period, scale, threshold, indices, frames, crop_pngs, var_delay, hq_colors, user_settings, quant_frames)
...
def extract_sprites(atlas_path, metadata_path, output_dir, create_gif, create_webp, set_framerate, set_loopdelay, set_minperiod, set_scale, set_threshold, set_indices, keep_frames, crop_pngs, var_delay, hq_colors, user_settings, quant_frames):
    frames_generated = 0
    anims_generated = 0
    sprites_failed = 0
    try:
