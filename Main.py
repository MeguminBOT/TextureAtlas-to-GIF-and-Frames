        ...
        if variable == input_dir:
            ...
            def on_select_png(evt):
                ...
                if os.path.isfile(os.path.join(directory, xml_filename)):
                    parse_xml(directory, xml_filename, listbox_data)
                elif os.path.isfile(os.path.join(directory, txt_filename)):
                    parse_txt(directory, txt_filename, listbox_data)
    ...
    if xml_files and png_files:
        ...
        def on_select_png(evt):
            ...
            if os.path.exists(xml_path):
                ...
                for name in names:
                    listbox_data.insert(tk.END, name)
...
import_menu.add_command(label="FNF: Import FPS from character json", command=lambda: fnf_select_char_json_directory(user_settings, xml_dict, listbox_png, listbox_data))
