...
class SparrowAtlasGenerator:
    ...
    def generate_atlas(
        ...
        try:
            ...
            xml_content = self._generate_sparrow_xml(
                output_path, atlas_width, atlas_height, current_version
            )
                ...
                for x in range(0, atlas_width - frame.width + 1, max(1, settings.padding)):
                    ...
                    if not any(rect.intersects(used) for used in self.used_rectangles):
                        ...
                        waste = self._calculate_waste(
                            x, y, frame.width, frame.height, atlas_width, atlas_height
                        )
    ...
    def _pack_maxrects(self, atlas_width: int, atlas_height: int, settings: AtlasSettings) -> bool:
        ...
        for frame in self.frames:
            ...
            for rect in self.free_rectangles:
                if (
                    rect.width >= frame.width + settings.padding * 2
                    and rect.height >= frame.height + settings.padding * 2
                ):
    ...
    def _split_free_rectangle(self, free_rect: Rectangle, used_rect: Rectangle):
        ...
        if used_rect.x > free_rect.x:
            self.free_rectangles.append(
                Rectangle(free_rect.x, free_rect.y, used_rect.x - free_rect.x, free_rect.height)
            )
        ...
        if used_rect.y > free_rect.y:
            self.free_rectangles.append(
                Rectangle(free_rect.x, free_rect.y, free_rect.width, used_rect.y - free_rect.y)
            )
    ...
    def _calculate_waste(
        self, x: int, y: int, width: int, height: int, atlas_width: int, atlas_height: int
    ) -> float:
    ...
    def _generate_sparrow_xml(
        self, output_path: str, atlas_width: int, atlas_height: int, current_version: str
    ) -> str:
        ...
        result_lines.append('<TextureAtlas imagePath="' + f"{Path(output_path).name}.png" + '">')
        ...
        for frame in sorted_frames:
            result_lines.append(
                ...
                + f'width="{frame.width}" height="{frame.height}" frameX="0" frameY="0" '
                + f'frameWidth="{frame.width}" frameHeight="{frame.height}" '
...
class AtlasGenerator(SparrowAtlasGenerator):
    ...
    def generate_atlas(
        self, input_frames: List[str], output_path: str, atlas_settings: dict, current_version: str
    ) -> dict:
        ...
        return super().generate_atlas(animation_groups, output_path, settings, current_version)
