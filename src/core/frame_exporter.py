    ...
    def save_frames(self, image_tuples, kept_frame_indices, spritesheet_name, animation_name, scale, settings):
        ...
        for index, frame in enumerate(image_tuples):
            if index in kept_frame_indices:
                ...
                frame_filename = os.path.join(frames_folder, f"{formatted_frame_name}{file_extension}")
                frame_image = frame[1]
                ...
                if bbox:
                    ...
                    self._save_frame_to_image(final_frame_image, frame_filename, frame_format, settings.get('compression_settings'))
                    frames_generated += 1
    ...
    def _save_frame_to_image(self, image, filename, frame_format, compression_settings=None):
        ...
        if frame_format == 'AVIF':
            ...
            if hasattr(image, 'info'):
                image.info['description'] = f'TIFF generated by TextureAtlas to GIF and Frames v{self.current_version}'
        ...
        try:
            ...
            try:
                ...
                metadata = PngInfo()
                metadata.add_text("Comment", f'PNG generated by TextureAtlas to GIF and Frames v{self.current_version}')
                image.save(png_filename, format='PNG', pnginfo=metadata, compress_level=9, optimize=True)
