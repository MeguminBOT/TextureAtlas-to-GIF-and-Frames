...
class ColorEffect:
    """Color effects for RGBA images exported by Adobe Animate."""
    ...
    def parse(cls, effect):
        mode = effect.get("M")
        if mode == "AD":
            multiplier = np.array([effect["RM"], effect["GM"], effect["BM"], effect["AM"]])
            ...
            if brightness < 0:
                multiplier = np.array([1 + brightness, 1 + brightness, 1 + brightness, 1])
                offset = np.zeros(4)
            else:
                multiplier = np.array([1 - brightness, 1 - brightness, 1 - brightness, 1])
            ...
            multiplier = np.array([1 - tint_multiplier, 1 - tint_multiplier, 1 - tint_multiplier, 1])
    ...
    def __call__(self, image):
        ...
        image = Image.fromarray((np.array(image) * multiplier + offset).clip(0, 255).astype("uint8"), mode="RGBA")
    ...
    def __eq__(self, other):
        ...
        return multiplier_self.tobytes() == multiplier_other.tobytes() and offset_self.tobytes() == offset_other.tobytes()
    ...
    def __matmul__(self, other):
        ...
        multiplier_self, offset_self = self.effect
        multiplier_other, offset_other = other.effect
        return ColorEffect((multiplier_self * multiplier_other, multiplier_self * offset_other + offset_self))
