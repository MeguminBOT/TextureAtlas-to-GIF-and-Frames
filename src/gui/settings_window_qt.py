...
"""
Qt-based Settings Window
Replaces the tkinter-based settings_window.py
"""
...
class SettingsWindow(QDialog):
    ...
    def __init__(self, parent, settings_manager):
        ...
        self.setWindowTitle("Current Settings Overview")
        self.setModal(True)
        self.resize(450, 350)
    ...
    def update_settings(self):
        ...
        animation_label = QLabel("Animation Settings")
        ...
        if self.settings_manager.animation_settings:
            for key, value in self.settings_manager.animation_settings.items():
                setting_label = QLabel(f"  {key}: {value}")
                setting_label.setStyleSheet("QLabel { margin-left: 20px; }")
                self.content_layout.addWidget(setting_label)
        else:
            no_settings_label = QLabel("  No animation-specific settings configured")
        ...
        spritesheet_label = QLabel("Spritesheet Settings")
        ...
        if self.settings_manager.spritesheet_settings:
            ...
            no_settings_label = QLabel("  No spritesheet-specific settings configured")
            no_settings_label.setStyleSheet(
                "QLabel { margin-left: 20px; color: #777; font-style: italic; }"
            )
            self.content_layout.addWidget(no_settings_label)
