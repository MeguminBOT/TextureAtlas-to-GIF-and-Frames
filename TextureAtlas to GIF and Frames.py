...
def process_directory(input_dir, output_dir, progress_var, tk_root, create_gif, create_webp, set_framerate):
    progress_var.set(0)
    ...
    for filename in os.listdir(input_dir):
        if filename.endswith('.png'):
            ...
            if os.path.isfile(xml_path):
                ...
                extract_sprites(os.path.join(input_dir, filename), xml_path, sprite_output_dir, create_gif, create_webp, set_framerate)
...
## Extraction logic
def extract_sprites(atlas_path, xml_path, output_dir, create_gif, create_webp, set_framerate):
    ...
    if create_gif:
        for animation_name, images in animations.items():
            images[0].save(os.path.join(output_dir, f"_{animation_name}.gif"), save_all=True, append_images=images[1:], disposal=2, optimize=False, duration=1000/set_framerate, loop=0)

    if create_webp:
        for animation_name, images in animations.items():
            images[0].save(os.path.join(output_dir, f"_{animation_name}.webp"), save_all=True, append_images=images[1:], disposal=2, optimize=False, duration=1000/set_framerate, loop=0)
