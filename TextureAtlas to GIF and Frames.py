...
def on_double_click_png(evt):
    ...
    def store_input():
        ...
        try:
            if frames_entry.get() != '':
                if not re.fullmatch(r',|all|first|last', frames_entry.get()):
...
def on_double_click_xml(evt):
    ...
    def store_input():
        ...
        try:
            ...
            messagebox.showerror("Invalid input", "Please enter a keyword or comma-separated list of integers or integer ranges for keep frame indices.")
            new_window.lift()
            return
        if len(anim_settings) > 0:
...
def create_scrollable_help_window():
    help_text = (
        ...
        "Keep Individual Frames:\nSelect the frames of the animation to save by typing 'all', 'first', 'last', or a comma-separated list of integers or integer ranges. Negative numbers count from the final frame.\n\n"
